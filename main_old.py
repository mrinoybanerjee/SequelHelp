...
# if the user has already been assigned a question then
# go to next question otherwise assign them a quesiton
# def get_next_question():
#     current_question = CurrentQuestion.query.filter_by(user_id=current_user.id).first()
#     total = run_query('SELECT count(*) FROM Questions', 'sequelhelp_mock')[0][0]
    
#     if (current_question):
#         question_obj = current_question
#         question_obj.question_id = (question_obj.question_id + 1) % (total+1)
        
#         if (question_obj.question_id == 0):
#             question_obj.question_id = 1
#         db.session.commit()
#     else:
#         question_obj = CurrentQuestion(user = current_user.id, question = 1)
#         db.session.add(question_obj)
#         db.session.commit()
        
#     question = run_query('SELECT Question FROM Questions WHERE QuestionId = ' + str(question_obj.question_id), 'sequelhelp_mock')[0][0]
#     question = {'id': question_obj.question_id, 'question': question}
#     return question
...
def get_next_question():
    ...
    # return {'id': result[0], 'question': result[1]}
    if (len(result)!=0):
        ...
        run_query('UPDATE current_question SET question_id=' + str(new_question_id) + ' WHERE question_id=' + str(question_id) + ';', 'sequelhelp_mock')
...
def parser_result(): 
    ...
    if(request.form.get('query')):
        ...
        if ('error' not in student_results):
            correct = True 
            question_id = CurrentQuestion.query.filter_by(user_id=current_user.id).first().question_id
